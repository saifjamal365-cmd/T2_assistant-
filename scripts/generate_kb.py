...
It writes ~1000 company-policy documents (English + Arabic) under
`data/knowledge_base/`, plus `manifest.csv` describing every file.

The corpus is:
  * built from ~50 hand-written topics across 8 departments,
  * expanded with per-office variants (numbers and scope differ),
  * seeded, so re-running produces the exact same corpus.

Run:  python scripts/generate_kb.py           (default: 1000 documents)
...
def main() -> None:
    ...
    ap.add_argument("--count", type=int, default=1000, help="number of documents (default 1000)")
